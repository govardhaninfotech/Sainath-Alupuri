...
async function loadItemsData() {
    ...
    const url = `${itemURLphp}?page=${currentItemsPage}&per_page=${itemsPerPage}`;
...
// ============================================
export async function renderItemsTable() {
    await loadItemsData();
    return generateItemsTableHTML();
}
...
function generateItemsTableHTML() {
    ...
    itemsData.forEach(item => {
        // Image cell:
        // - Try to load image_path
        // - If it fails (404), hide img and show "No image"
        const imageCell = `
            ...
            <tr>
                ...
                <td style="width: 150px;">
                    <div style="display: flex; align-items: center; justify-content: center;">
                        ...
                        <span class="status-text" style="margin-left: 10px; font-weight: 500; min-width: 60px;"></span>
                    </div>
                </td>
                <td>
...
// ============================================
async function changeItemPage(direction) {
...
async function changeItemPerPage(value) {
...
function editItem(id) {
    ...
    statusText.textContent = item.status === "enable" ? "Enable" : "Disable";
...
async function submitItemForm(event) {
    ...
    if (editingItemId) {
        ...
        const result = await updateItem(itemURLphp, editingItemId, formData);
        if (result) {
        ...
        const result = await addItemToAPI(itemURLphp, formData);
        if (result) {
...
async function toggleItemStatusItems(id, currentStatus) {
    const confirmed = await showConfirm(
    ...
    const result = await updateItem(itemURLphp, id, { status: newStatus });
