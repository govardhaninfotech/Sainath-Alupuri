...
async function loadclientData() {
    clientData = await getItemsData(userURLphp);
...
// ============================================
async function toggleclienttatus(id) {
    ...
    // Update in API
    const result = await updateclienttatus(item.id, newStatus);
    if (result) {
...
async function submitclientForm(event) {
    ...
    if (editingItemId) {
        ...
        const result = await updateItem(editingItemId, formData);
        if (result) {
        ...
        const result = await addItemToAPI(newItemData);
        if (result) {
...
async function deleteclient(id) {
    ...
    const result = await deleteItemFromAPI(item.id);
    if (result) {
